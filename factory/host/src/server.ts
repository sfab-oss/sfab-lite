 ...
 *
 * Host routes (/api, /agents, /a/, /kernel, /mcp, …) stay on this
 * dispatch; unmatched document paths fall through to Start.
 * Create returns `202` with a create-job id; poll `GET .../attempts/:id`.
 * Shipping code is PR merge onto `main` (CD writes an immutable build and
 * sets D1 `live_sha`). Protected API credentials: `X-Admin-Token` or
 * session. See `hono/tenancy.ts`. Domain handlers in `lib/protected/`;
 * Hono in `hono/protected/` and `hono/host.ts`; CD in `forge/cd.ts`;
 * code host in `code-host/`.
 */
 ...
 */
async function dispatchFactoryRequest(
  request: Request,
  env: Env,
