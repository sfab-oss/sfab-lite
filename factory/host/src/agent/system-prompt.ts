...
export function buildSystemPrompt(opts: {
  ...
  return [
    ...
    "",
    "What you can import:",
    "  The kernel serves a fixed set of modules. There is no install step,",
    "  and package.json is generated — not a place to add packages. There is",
    "  no node_modules directory on disk either — kernel modules are served",
    "  at resolution time, so there is nothing to list, grep or introspect.",
    "  The lists below are the kernel inventory.",
    "",
    "  Catalog modules are a fourth path, enabled only by apps_add of a",
    "  recipe that lists a catalog pin (today: apps_add lite/pdf-invoice",
    "  for pdf-lib, or apps_add lite/xlsx-export for exceljs). Do not edit",
    "  manifest.modules yourself; the host writes it from the recipe.",
    "  Unknown npm names are still forbidden.",
    "",
    "  Typecheck enforces the closed import surface. A specifier the kernel",
    "  does not serve — and that is not a declared catalog module — fails",
    "  with a LITE-RESOLVE diagnostic, including transitive-only types",
    "  (kysely, jose, better-call and others) that exist so vendor .d.ts can",
    "  resolve, not so apps can import them. Trust the lists below. Do not",
    "  invent declare module stubs to make an out-of-map package look typed.",
    "  If the user asks for a package that is neither on these lists nor",
    "  enabled by such a recipe, say you cannot use it and stop. Do not",
    "  pnpm add it, and do not ship the import anyway.",
    "",
