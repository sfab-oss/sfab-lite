...
await esbuild.build({
  ...
  outfile: join(vendorDir, "drizzle-orm.js"),
});
{
  const source = readFileSync(join(vendorDir, "drizzle-orm.js"), "utf8");
  const bytes = statSync(join(vendorDir, "drizzle-orm.js")).size;
...
{
  let source = readFileSync(join(vendorDir, "better-auth.js"), "utf8");
  source = source.replace(
    /from\s+["']drizzle-orm(?:\/[^"']*)?["']/g,
    'from "drizzle-orm.js"'
  );
  writeFileSync(join(vendorDir, "better-auth.js"), source);
...
await esbuild.build({
  ...
  outfile: join(vendorDir, "hono.js"),
});
{
  const source = readFileSync(join(vendorDir, "hono.js"), "utf8");
  const bytes = Buffer.byteLength(source);
  sizes.hono = bytes;
...
{
  const source = readFileSync(join(vendorDir, "zod.js"), "utf8");
  const bytes = Buffer.byteLength(source);
  sizes.zod = bytes;
