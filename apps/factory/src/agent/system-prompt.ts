...
export function buildSystemPrompt(opts: {
  ...
  return [
    ...
    "Ship by committing on a feature branch, pushing it, opening a PR, waiting for checks, then merging.",
    ...
    "  gh run list|view|watch|rerun         — check runs (virtual gh)",
    ...
    "  branch, `git push origin <branch>`, `gh pr create --title …`, wait for checks",
    "  (`gh pr checks` / `gh run watch`), then `gh pr merge`. Nothing reaches live",
    "  until the PR merges and CD advances main.",
  ].join("\n");
}
